...
const AdminServices = () => {
  ...
  const { data:services, isLoading, isError } = useQuery(
    ['admin-services',search],
    async ()=>{
      const res = await api.get('/api/services',{ params:{ search, limit:100 }});
      return res.data.data.services;
    }
  );
  ...
  const updateMutation = useMutation(
    updated => api.put(`/api/services/${editingId}`, updated),
    {
      onSuccess:()=>{
  ...
  const toggleDay = day=>{
    setForm(p=>({
      ...
      operatingHours:{
        ...p.operatingHours,
        [day]:{ ...p.operatingHours[day], isOpen:!p.operatingHours[day].isOpen }
      }
    }));
  };
